...
export default function Chat({ 
  ...
  // Inteligência de Segurança e Sessão
  ...
  const handleSelecionarContato = (contato) => {
    ...
    setTimeout(() => setIsHandshaking(false), 600);
  ...
  const dispararMensagem = (textoFinal) => {
    ...
    const novaMsg = { id: Date.now(), remetenteId: userId, remetenteNome: nomeLogado, destinoId: contatoAtivo.id, texto: textoComSeguranca, data: new Date(), tipo: 'sent' };
  ...
  const enviarMensagemTexto = (e) => {
    ...
    dispararMensagem(textoFinal); setMensagem(''); setResponderA(null); setIsConfidential(false);
  };
  ...
  const handleDragOver = (e) => { e.preventDefault(); if (contatoAtivo && !isHandshaking && !(contatoAtivo.isGroup && !isAdminOrDev)) setIsDragging(true); };
  const handleDragLeave = (e) => { e.preventDefault(); setIsDragging(false); };
  const handleDrop = (e) => { e.preventDefault(); setIsDragging(false); if (!contatoAtivo || isHandshaking || (contatoAtivo.isGroup && !isAdminOrDev)) return; processFile(e.dataTransfer.files[0]); };
  ...
  const renderBubbleText = (msg) => {
    ...
    if (textoBruto.startsWith('[CALL_END]')) { return wrapConfidential(<div className="system-msg-bubble" style={{background: 'rgba(56, 189, 248, 0.1)', color: 'var(--chat-secondary)', borderColor: 'rgba(56, 189, 248, 0.3)'}}><PhoneOff size={16} /> {textoBruto.replace('[CALL_END]', '')}</div>); }
    if (textoBruto.startsWith('[LOCATION]')) { return wrapConfidential(<div className="gps-location-bubble"><div className="gps-icon"><MapPin size={24} /></div><div className="gps-details"><strong>Coordenadas Táticas</strong><span>{textoBruto.replace('[LOCATION]', '').trim()}</span></div></div>); }
    ...
    if (textoBruto.startsWith('[FILE:')) {
      ...
      if (fileType.startsWith('image/')) { return wrapConfidential(<div className="file-img-bubble"><img src={src} alt={fileName} className="chat-img-thumbnail" onClick={() => setPreviewImage(src)} /><span style={{fontSize: '0.75rem', fontWeight: 'bold'}}>{fileName}</span></div>); } 
      else { return wrapConfidential(<a href={src} download={fileName} className="chat-file-attachment"><FileText size={20} color="var(--chat-primary)" /><span>{fileName}</span></a>); }
    }
    ...
    if (repMatch) {
      ...
      return wrapConfidential(<><div className="msg-reply-block"><strong>Citação Direta:</strong><p>{repliedContent}</p></div>{repMatch[2]}</>);
    }
  ...
  return (
    <div className={`anim-fade-in chat-page-container ${contatoAtivo ? 'has-active-chat' : ''}`} onClick={() => { setShowCommands(false); setShowAttachMenu(false); setShowEncryptionInfo(false); }} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
      
      {isDragging && contatoAtivo && !isHandshaking && !(contatoAtivo.isGroup && !isAdminOrDev) && (<div className="chat-drag-overlay"><div className="drag-content"><UploadCloud size={64} /><h2>Transmitir Pacote</h2><p>Solte para fazer upload no canal de {contatoAtivo.nome}</p></div></div>)}
      ...
      <div className="chat-sidebar">
        ...
        <div className="chat-contacts-list">
          {(!pesquisa || canalGlobal.nome.toLowerCase().includes(pesquisa.toLowerCase())) && (<div className={`chat-contact-item channel-global ${contatoAtivo?.id === 'todos' ? 'active' : ''}`} onClick={() => handleSelecionarContato(canalGlobal)}><div className="contact-avatar-wrapper"><div className="contact-avatar global-avatar"><Globe size={22} /></div></div><div className="contact-info"><span className="contact-name">{canalGlobal.nome}</span><span className="contact-role" style={{color: isAdminOrDev ? 'var(--chat-primary)' : 'var(--chat-muted)'}}>{canalGlobal.cargo}</span></div></div>)}
        ...
        {contatoAtivo ? (
          <>
            <div className="chat-main-header">
              {showSearchChat ? (
                ...
                <div className="chat-active-user anim-fade-in" onClick={() => !contatoAtivo.isGroup && setShowAgentModal(true)} style={{cursor: contatoAtivo.isGroup ? 'default' : 'pointer'}}>
                  ...
                  <div className="chat-user-header-details"><h3>{contatoAtivo.nome}</h3>{isTyping && !contatoAtivo.isGroup ? <span className="chat-status-typing">Criptografando pacote...</span> : <span className="chat-status-online"><span className="chat-status-dot"></span> {contatoAtivo.isGroup ? 'Rede Unificada' : 'Conexão Segura Estabelecida'}</span>}</div>
                </div>
              )}
            ...
            <div className="secure-channel-banner" onClick={(e) => { e.stopPropagation(); setShowEncryptionInfo(!showEncryptionInfo); }}>
              ...
              {showEncryptionInfo && (
                <div className="anim-slide-up" style={{position: 'absolute', top: '100%', left: '50%', transform: 'translateX(-50%)', background: 'rgba(2, 6, 23, 0.95)', border: '1px solid var(--chat-primary)', padding: '15px', borderRadius: '12px', zIndex: 100, width: '300px', boxShadow: '0 15px 30px rgba(0,0,0,0.8)', color: 'white', textTransform: 'none', fontFamily: 'Montserrat', textAlign: 'left', lineHeight: '1.5'}}>
                  ...
                  <div style={{fontSize: '0.7rem', color: 'var(--chat-muted)', fontFamily: 'JetBrains Mono', background: 'rgba(0,0,0,0.5)', padding: '6px', borderRadius: '6px'}}>Session Key: 0x8F9B...3A12<br/>TLS 1.3 Cipher: TLS_AES_256_GCM</div>
                </div>
              )}
            </div>
            ...
            {pinnedMessage && (
              <div className="pinned-message-banner anim-slide-up" style={{ background: 'rgba(56, 189, 248, 0.1)', borderBottom: '1px solid rgba(56, 189, 248, 0.3)', padding: '10px 15px', display: 'flex', alignItems: 'flex-start', gap: '10px', color: 'white', cursor: 'pointer' }}>
                 ...
                 <div style={{ flex: 1, overflow: 'hidden' }}>
                    ...
                    <div style={{ fontSize: '0.85rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{pinnedMessage.texto.replace(/\[AUDIO\].*/, '🎤 Áudio').replace(/\[FILE:.*?\].*/, '📎 Anexo').replace(/\[LOCATION\].*/, '📍 Localização').replace(/\[CONFIDENCIAL\]\s*/, '')}</div>
                 </div>
              ...
              {isHandshaking ? (
                ...
                <div className="chat-history" onScroll={handleScroll} ref={historyContainerRef}>
                  ...
                  {mensagensExibidas.map((msg, index) => {
                    ...
                    const isConfidentialMsg = msg.texto?.includes('[CONFIDENCIAL]');
            ...
            <div className="chat-input-container">
              {/* Bloqueio do Canal Global para Usuários de Loja */}
              {contatoAtivo.isGroup && !isAdminOrDev ? (
                ...
                <>
                  ...
                  {responderA && (<div className="reply-context-box"><div className="reply-info"><strong>Citação de {responderA.remetenteNome.split(' ')[0]}</strong><p>{responderA.texto.replace(/\[AUDIO\].*/, '🎤 Áudio').replace(/\[FILE:.*?\].*/, '📎 Anexo').replace(/\[LOCATION\].*/, '📍 Localização').replace(/\[CONFIDENCIAL\]\s*/, '').replace(/\[REP:.*?\]\s*/, '')}</p></div><button type="button" className="btn-close-reply" onClick={() => setResponderA(null)}><X size={18} /></button></div>)}
